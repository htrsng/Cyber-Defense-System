...
async function bruteForce(req, res, io) {
    try {
        ...
        return res.json({
            message: `Brute force attack simulated with ${attempts} attempts`,
            attempts,
            finalRiskScore,
            level,
            blocked,
            reasons: risk.reasons,
            results: Array.from({ length: attempts }, (_, i) => ({
                attempt: i + 1,
                payload: `fakepass${i + 1}`,
                status: 401,
                blocked: true,
            })),
        });
        ...
        return res.status(500).json({ error: 'Failed to run brute force simulation', details: error.message });
    }
}
    ...
    try {
        ...
        for (const payload of payloads) {
            try {
                ...
                console.warn(`SQLi test error for payload "${payload}":`, error.message);
                await ActivityLog.create({
                    eventType: 'ATTACK_SIM_SQLI',
                    ipAddress: attackerIp,
                    severity: 'high',
        ...
        return res.json({
            message: `SQL Injection attack simulated with ${payloads.length} payloads`,
        ...
        return res.status(500).json({ error: 'Failed to run SQLi simulation', details: error.message });
    }
}
